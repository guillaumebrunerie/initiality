...
data respectsCtx : {n : ℕ} (X : Ob n) (Γ : Ctx n) → Prop where
  tt : {X : _} → respectsCtx X ◇
  _,_ : {n : ℕ} {X : _} {Γ : _} {A : _} → respectsCtx {n} (ft X) Γ → Σ (isDefined (⟦ A ⟧Ty (ft X))) (λ Aᵈ → ⟦ A ⟧Ty (ft X) $ Aᵈ ≡ X) → respectsCtx X (Γ , A)
...
⟦weakenTy⟧ᵈ'prev : (k : Fin (suc n)) {X+ : Ob (suc (suc n -F' (prev k)))} {X : Ob (suc n)} {X' : Ob n} (A : TyExpr (suc n))
            → isDefined (⟦ A ⟧Ty X)
            → (X= : ft X+ ≡ ft^ k X') {Y : Ob (suc (suc n))}
            → (p : ft X ≡ X')
            → (Y= : star (qq^ k X=) X (qq^₁ k X= ∙ ! p) ≡ Y)
            → isDefined (⟦ weakenTy' (prev k) A ⟧Ty Y)
⟦weakenTy⟧ᵈ'prev k A Aᵈ X= refl refl = ⟦weakenTy⟧ᵈ' (prev k) A Aᵈ X= refl
...
⟦weakenTy⟧='prev : (k : Fin (suc n)) {X+ : Ob (suc (suc n -F' (prev k)))} {X : Ob (suc n)} {X' : Ob n} (A : TyExpr (suc n))
            → (Aᵈ : isDefined (⟦ A ⟧Ty X))
            → (X= : ft X+ ≡ ft^ k X') {Y : Ob (suc (suc n))}
            → (p : ft X ≡ X')
            → (Y= : star (qq^ k X=) X (qq^₁ k X= ∙ ! p) ≡ Y)
            → star (qq (qq^ k (X= ∙ ap (ft^ k) (! p))) X (qq^₁ k _)) (⟦ A ⟧Ty X $ Aᵈ) (qq₁ ∙ ! (⟦⟧Ty-ft A)) ≡ ⟦ weakenTy' (prev k) A ⟧Ty Y $ ⟦weakenTy⟧ᵈ'prev k A Aᵈ X= p Y=
⟦weakenTy⟧='prev k A Aᵈ X= refl refl = ap2-irr star (! qq^prev) refl ∙ ⟦weakenTy⟧=' (prev k) A Aᵈ X= refl
...
⟦weakenTm⟧='prev : (k : Fin (suc n)) {X+ : Ob (suc (suc n -F' (prev k)))} {X : Ob (suc n)} {X' : Ob n} (u : TmExpr (suc n))
            ...
            → (p : ft X ≡ X')
            → (Y= : star (qq^ k X=) X (qq^₁ k X= ∙ ! p) ≡ Y)
            → starTm (qq^ (prev k) (X= ∙ ap (ft^ k) (! p))) (⟦ u ⟧Tm X $ uᵈ) (⟦⟧Tm₀ u ∙ ! (qq^₁ (prev k) (X= ∙ ap (ft^ k) (! p)))) ≡ ⟦ weakenTm' (prev k) u ⟧Tm Y $ ⟦weakenTm⟧ᵈ'prev k u uᵈ X= p Y=
⟦weakenTm⟧='prev k u uᵈ X= refl refl = ⟦weakenTm⟧=' (prev k) u uᵈ X= refl
...
⟦weakenTm⟧₁'prev k u uᵈ X= refl refl u₁ = ⟦weakenTm⟧₁' (prev k) u uᵈ X= refl u₁ ∙ ap2-irr star (qq^prev ∙ ap2-irr qq refl (! (⟦⟧Tm₁-ft u) ∙ ap ft u₁)) refl
...
respectsCtxExt r A {Aᵈ} rewrite ⟦⟧Ty-ft A {Aᵈ} = {!r!} , {!_ , !}
...
⟦⟧Tmᵈ r (VarLast dA) = tt
⟦⟧Tmᵈ (r , _) {u = var (prev x)} (VarPrev dA dx) = tt
...
⟦⟧Tmᵈ r {u = natelim P dO dS u} (Natelim dP ddO ddS du) =
  ...
  let thing = ⟦subst⟧Tyᵈ (NatStr= ∙ ! (⟦⟧Ty-ft P {Aᵈ = Pᵈ})) (weakenTy' (prev last) P) (⟦weakenTy⟧ᵈ' (prev last) P Pᵈ refl (ap NatStr (! pp₀) ∙ ! (ap2-irr star qq^last refl ∙ NatStrNat _ (! NatStr= ∙ ! pp₁)))) (suc (var last)) (tt , ssₛ , (ss₁' (id₁ ∙ ⟦⟧Ty-ft P) ∙ NatStrNat _ (! (comp₁ ∙ pp₁ ∙ NatStr=)) ∙ ap NatStr (comp₀ ∙ ! ss₀)) , tt) (sucStr₁ ∙ ap NatStr (ss₀ ∙ id₀ ∙ ⟦⟧Ty-ft P)) in
  (Pᵈ ,
   ⟦⟧Ty-ft P ,
   ⟦⟧Tmᵈ r ddO ,
   ⟦⟧Tmₛ dO ,
   ...
   (⟦⟧Tm₁ (respectsCtxExt {Γ = _ , nat} (respectsCtxExt r nat) P {Pᵈ}) {Aᵈ = ⟦weakenTy⟧ᵈ' last (substTy (weakenTy' (prev last) P) (suc (var last))) thing refl refl} ddS
    ...
    ∙ ap2-irr star qq^last (⟦subst⟧Ty= (NatStr= ∙ ! (⟦⟧Ty-ft P {Aᵈ = Pᵈ})) (weakenTy' (prev last) P) (⟦weakenTy⟧ᵈ'prev last P Pᵈ refl refl (NatStrNat _ (! NatStr= ∙ ! (qq^₁ last refl)) ∙ ap NatStr (qq^₀ last refl))) (suc (var last)) (tt , ssₛ , (ss₁' (id₁ ∙ ⟦⟧Ty-ft P) ∙ NatStrNat _ (! (comp₁ ∙ pp₁ ∙ NatStr=)) ∙ ap NatStr (comp₀ ∙ ! ss₀)) , tt) (sucStr₁ ∙ ap NatStr (ss₀ ∙ id₀ ∙ ⟦⟧Ty-ft P))
      ∙ ap2-irr star refl (! (⟦weakenTy⟧='prev last P Pᵈ refl refl (NatStrNat _ (! NatStr= ∙ ! (qq^₁ last refl)) ∙ ap NatStr (qq^₀ last refl))) ∙ ap2-irr star (ap2-irr qq qq^last refl) refl))) ,
   ⟦⟧Tmᵈ r du ,
   ⟦⟧Tmₛ u ,
   (⟦⟧Tm₁ r du ∙ ! (⟦⟧Ty-ft P)) , tt)
...
⟦⟧Tmᵈ r {u = refl A a} (Refl dA da) =
  (⟦⟧Tyᵈ r dA ,
   ...
   ⟦⟧Tmₛ a ,
   ⟦⟧Tm₁ r da , tt)
⟦⟧Tmᵈ {Γ = Γ} {X = X} r {u = jj A P d a b p} (JJ dA dP dd da db dp) =
   (Aᵈ ,
    Pᵈ ,
    ⟦⟧Ty-ft P ,
    ⟦⟧Tmᵈ (respectsCtxExt r A) dd ,
    ⟦⟧Tmₛ d ,
    (⟦⟧Tm₁ (respectsCtxExt r A) {Aᵈ = ⟦tsubst⟧Tyᵈ P Pᵈ (((weakenMor' last (idMor _) , var last) , var last) , refl (weakenTy' last A) (var last)) {!!}} dd
    ...
    ⟦⟧Tmᵈ r dp ,
    ⟦⟧Tmₛ p ,
    ⟦⟧Tm₁ r {A = id A a b} {Aᵈ = Aᵈ , (⟦⟧Tmᵈ r da , ⟦⟧Tmₛ a , ⟦⟧Tm₁ r da , ⟦⟧Tmᵈ r db , ⟦⟧Tmₛ b , ⟦⟧Tm₁ r db , tt)} dp , tt)
    {- (ap2-irr comp (ap2-irr qq (⟦idMor+⟧= (ap ft (ft-star ∙ pp₀ ∙ ⟦weakenTy⟧=' last A Aᵈ (⟦⟧Ty-ft A) refl) ∙ ⟦⟧Ty-ft (weakenTy A)) (var last) tt (ss₁' id₁ ∙ {!? ∙ !} ∙ ! (ft-star ∙ pp₀))) refl) (ap-irr-reflStr (! (⟦weakenTy⟧=' last A Aᵈ (⟦⟧Ty-ft A) refl)) refl))-} where
     ...
     abstract
      ...
      Pᵈ = {-cong⟦⟧Ty {X = IdStr
                           (⟦ weakenTy (weakenTy A) ⟧Ty
                            (⟦ weakenTy A ⟧Ty
                             (star^ last (⟦ A ⟧Ty (ft^ last X) $ Aᵈ) X (⟦⟧Ty-ft A))
                             $ ⟦weakenTy⟧ᵈ' last A (⟦⟧Tyᵈ r dA) (⟦⟧Ty-ft A) refl)
                            $
                            ⟦weakenTy⟧ᵈ' last (weakenTy A)
                            (⟦weakenTy⟧ᵈ' last A (⟦⟧Tyᵈ r dA) (⟦⟧Ty-ft A) refl)
                            (⟦⟧Ty-ft (weakenTy A)) refl)
                           (⟦ var (prev last) ⟧Tm
                            (⟦ weakenTy A ⟧Ty (⟦ A ⟧Ty X $ Aᵈ) $
                             ⟦weakenTy⟧ᵈ' last A (⟦⟧Tyᵈ r dA) (⟦⟧Ty-ft A) refl)
                            $ (tt , {!ss-id₁!} , tt))
                           ssₛ {!!}
                           (⟦ var last ⟧Tm
                            (⟦ weakenTy A ⟧Ty (⟦ A ⟧Ty X $ Aᵈ) $
                             ⟦weakenTy⟧ᵈ' last A (⟦⟧Tyᵈ r dA) (⟦⟧Ty-ft A) refl)
                            $ tt)
                           ssₛ {!ss-id₁!}} {A = P} (ap-irr-IdStr (! (⟦weakenTy⟧= (weakenTy A) (⟦weakenTy⟧ᵈ' last A Aᵈ (⟦⟧Ty-ft A) refl) (⟦⟧Ty-ft (weakenTy A)) refl) ∙ ap2-irr star (ap pp (! (⟦weakenTy⟧= A Aᵈ (⟦⟧Ty-ft A) refl))) (! (⟦weakenTy⟧= A Aᵈ (⟦⟧Ty-ft A) refl))) (ap-irr (λ z p → ⟦ var (prev last) ⟧Tm z $ p) (! (⟦weakenTy⟧= A Aᵈ (⟦⟧Ty-ft A) refl)) ∙ ap ss (ap2-irr comp (ap ss (ap idC (ft-star ∙ pp₀))) refl) ∙ ss-comp {g = pp (⟦ A ⟧Ty X $ ⟦⟧Tyᵈ r dA)} ∙ ap ss (! (assoc {q = {!!}}) ∙ ap2-irr comp (ap2-irr comp (ap2-irr qq (! (id-left' (pp₀ ∙ id₁) ∙ ap pp id₁)) refl) refl ∙ ! ss-qq) refl ∙ id-right' (pp₁ ∙ ft-star ∙ pp₀))) ((ap-irr (λ z p → ⟦ var last ⟧Tm z $ p) (! (⟦weakenTy⟧= A Aᵈ (⟦⟧Ty-ft A) refl)))))-} cong⟦⟧Ty {A = P} (ap-irr-IdStr (! (ap2-irr star (ap pp AA=) AA= ∙ AAA=)) (ap ss (ap pp (! AA=))) (ap ss (ap idC (! AA=)))) Pᵈ'
{-
((ss₁' (comp₁ ∙ ss₁' (id₁ ∙ ⟦⟧Ty-ft (weakenTy A)) ∙ ap2-irr star (id-left' (pp₀ ∙ ! (⟦⟧Ty-ft (weakenTy A)))) refl ∙ ⟦weakenTy⟧=' last A Aᵈ (⟦⟧Ty-ft A) refl) ∙ ap2-irr star (! assoc ∙ ap2-irr comp (ap2-irr comp (ap pp (! (⟦weakenTy⟧=' last A Aᵈ (⟦⟧Ty-ft A) refl) ∙ ap2-irr star (! (id-left' (pp₀ ∙ id₁) ∙ ap pp id₁)) (! id₁))) (ap ss (ap idC (⟦⟧Ty-ft (weakenTy A)))) ∙ ss-pp) refl ∙ id-right' (pp₁ ∙ ⟦⟧Ty-ft (weakenTy A) ∙ ! id₀)) refl) ∙ (⟦weakenTy⟧=' last (weakenTy A) (⟦weakenTy⟧ᵈ' last A Aᵈ (⟦⟧Ty-ft A) refl) (⟦⟧Ty-ft (weakenTy A)) refl))
  (Aᵈ ,
   {!Pᵈ!} ,
   {!!} ,
   ⟦⟧Tmᵈ (respectsCtxExt r A) dd ,
   ⟦⟧Tmₛ d ,
   ? {-(⟦⟧Tm₁ (respectsCtxExt r A) dd ∙ ! (⟦tsubst⟧Ty= P {!!} (((weakenMor (idMor _) , var last) , var last) , refl (weakenTy A) (var last)) {!!}) ∙ {!!})-} ,
   ⟦⟧Tmᵈ r da ,
   ⟦⟧Tmₛ a ,
   ⟦⟧Tm₁ r da ,
   ⟦⟧Tmᵈ r db ,
   ...
   ⟦⟧Tmₛ p ,
   ⟦⟧Tm₁ r dp , tt)-}
{- (respectsCtxExt {Γ = ((Γ , A) , weakenTy A)} (respectsCtxExt {Γ = (Γ , A)} (respectsCtxExt {Γ = Γ} r A {Aᵈ = Aᵈ}) (weakenTy A) {Aᵈ = ⟦weakenTy⟧ᵈ' last A Aᵈ (⟦⟧Ty-ft A) refl}) (id (weakenTy (weakenTy A)) (var (prev last)) (var last))
           {Aᵈ = (⟦weakenTy⟧ᵈ' last (weakenTy A) (⟦weakenTy⟧ᵈ' last A Aᵈ (⟦⟧Ty-ft A) refl) (⟦⟧Ty-ft (weakenTy A)) refl ,
                  ⟦⟧Tmᵈ (respectsCtxExt {Γ = Γ , A} (respectsCtxExt {Γ = Γ} r A {Aᵈ = Aᵈ}) (weakenTy A) {Aᵈ = ⟦weakenTy⟧ᵈ' last A Aᵈ (⟦⟧Ty-ft A) refl} ) (VarPrev {B = weakenTy A} (WeakTy A dA) (VarLast dA)) ,
                  ⟦⟧Tmₛ (var (prev last)) {uᵈ =  ⟦⟧Tmᵈ (respectsCtxExt {Γ = Γ , A} (respectsCtxExt {Γ = Γ} r A {Aᵈ = Aᵈ}) (weakenTy A) {Aᵈ = ⟦weakenTy⟧ᵈ' last A Aᵈ (⟦⟧Ty-ft A) refl} ) (VarPrev {B = weakenTy A} (WeakTy A dA) (VarLast dA))} ,
                  ⟦⟧Tm₁ {Γ = (Γ , A) , weakenTy A} (respectsCtxExt {Γ = Γ , A} (respectsCtxExt {Γ = Γ} r A {Aᵈ = Aᵈ}) (weakenTy A) {Aᵈ = ⟦weakenTy⟧ᵈ' last A Aᵈ (⟦⟧Ty-ft A) refl}) {uᵈ = ⟦⟧Tmᵈ (respectsCtxExt {Γ = Γ , A} (respectsCtxExt {Γ = Γ} r A {Aᵈ = Aᵈ}) (weakenTy A) {Aᵈ = ⟦weakenTy⟧ᵈ' last A Aᵈ (⟦⟧Ty-ft A) refl} ) (VarPrev {B = weakenTy A} (WeakTy A dA) (VarLast dA))} (VarPrev (WeakTy A dA) (VarLast dA)) ,
                  ⟦⟧Tmᵈ (respectsCtxExt {Γ = Γ , A} (respectsCtxExt {Γ = Γ} r A {Aᵈ = Aᵈ}) (weakenTy A) {Aᵈ = ⟦weakenTy⟧ᵈ' last A Aᵈ (⟦⟧Ty-ft A) refl}) (VarLast (WeakTy A dA)) ,
                  ⟦⟧Tmₛ (var last) ,
                  ⟦⟧Tm₁ (respectsCtxExt {Γ = Γ , A} (respectsCtxExt {Γ = Γ} r A) (weakenTy A) {Aᵈ = ⟦weakenTy⟧ᵈ' last A Aᵈ (⟦⟧Ty-ft A) refl} ) (VarLast (WeakTy A dA)) , tt)})-}
...
-- ⟦⟧Tm₁ r (VarLast {A = A} dA) =
--   ss₁ ∙ ap2-irr star (ap2-irr comp (ap pp id₁) (ap idC (! pp₀)) ∙ id-left ∙ ! qq^last) (id₁ ∙ ! (snd (snd r)))
...
-- ⟦⟧Tm₁ r {u = natelim P dO dS u} (Natelim dP ddO ddS du) = natelimStr₁ ∙ ! (⟦subst⟧Ty= NatStr= P (⟦⟧Tyᵈ (respectsCtxExt r nat) dP) u (⟦⟧Tmᵈ r du) (⟦⟧Tm₁ r du))
...
--    ⟦weakenTy⟧ᵈ' (prev k) B Bᵈ (X= ∙ ap (ft^ k) (! (⟦⟧Ty-ft A))) (! (⟦weakenTy⟧=' k A Aᵈ X= Y=) ∙ ap2-irr star (qq^=p (! (⟦⟧Ty-ft A))) refl) , tt)
...
--    (⟦weakenTm⟧₁'prev k u uᵈ X= (⟦⟧Ty-ft A) (⟦weakenTy⟧=' k A Aᵈ X= Y=) u₁ ∙ ap2-irr star (ap2-irr qq (qq^=p (! (⟦⟧Ty-ft A))) (⟦⟧Ty-ft B)) refl
--      ∙ ⟦weakenTy⟧='prev k B Bᵈ (X= ∙ ap (ft^ k) (! (⟦⟧Ty-ft A))) refl (ap2-irr star (qq^=p (⟦⟧Ty-ft A)) refl ∙ ⟦weakenTy⟧=' k A Aᵈ X= Y=)) , tt)
-- ⟦weakenTm⟧ᵈ' k (app A B f a) (Aᵈ , Bᵈ , fᵈ , fₛ , f₁ , aᵈ , aₛ , a₁ , tt) X= Y= =
--   (⟦weakenTy⟧ᵈ' k A Aᵈ X= Y= ,
--    ⟦weakenTy⟧ᵈ' (prev k) B Bᵈ (X= ∙ ap (ft^ k) (! (⟦⟧Ty-ft A))) (! (⟦weakenTy⟧=' k A Aᵈ X= Y=) ∙ ap2-irr star (qq^=p (! (⟦⟧Ty-ft A))) refl) ,
--    ⟦weakenTm⟧ᵈ' k f fᵈ X= Y= ,
--    ⟦⟧Tmₛ (weakenTm' k f) ,
--    (⟦weakenTm⟧₁' k f fᵈ X= Y= f₁ ∙ PiStrNat (qq^ k X=) (ap ft (⟦⟧Ty-ft B) ∙ ⟦⟧Ty-ft A ∙ ! (qq^₁ k X=)) ∙ ap PiStr (ap2-irr star (ap2-irr qq (qq^=p (! (⟦⟧Ty-ft A))) (⟦⟧Ty-ft B)) refl ∙ ⟦weakenTy⟧='prev k B Bᵈ (X= ∙ ap (ft^ k) (! (⟦⟧Ty-ft A))) refl (ap2-irr star (qq^=p (⟦⟧Ty-ft A)) refl ∙ ⟦weakenTy⟧=' k A Aᵈ X= Y=))) ,
--    ⟦weakenTm⟧ᵈ' k a aᵈ X= Y= ,
--    ⟦⟧Tmₛ (weakenTm' k a) ,
--    (⟦weakenTm⟧₁' k a aᵈ X= Y= a₁ ∙ ! (ft-star ∙ qq^₀ (prev k) (_) ∙ ap2-irr star (qq^=p (⟦⟧Ty-ft A)) (! (⟦⟧Ty-ft B))) ∙ ap ft (⟦weakenTy⟧=' (prev k) B Bᵈ (X= ∙ ap (ft^ k) (! (⟦⟧Ty-ft A))) (! (⟦weakenTy⟧=' k A Aᵈ X= Y=) ∙ ap2-irr star (qq^=p (! (⟦⟧Ty-ft A))) refl))) , tt)
...
--    ⟦weakenTm⟧ᵈ' k a aᵈ X= Y= ,
--    ⟦⟧Tmₛ (weakenTm' k a) ,
--    (⟦weakenTm⟧₁' k a aᵈ X= Y= a₁ ∙ ! (ft-star ∙ qq^₀ (prev k) _ ∙ ap2-irr star (qq^=p (⟦⟧Ty-ft A)) (! (⟦⟧Ty-ft B))) ∙ ap ft (⟦weakenTy⟧=' (prev k) B Bᵈ (X= ∙ ap (ft^ k) (! (⟦⟧Ty-ft A))) (! (⟦weakenTy⟧=' k A Aᵈ X= Y=) ∙ ap2-irr star (qq^=p (! (⟦⟧Ty-ft A))) refl))) ,
--    ⟦weakenTm⟧ᵈ' k b bᵈ X= Y= ,
--    ⟦⟧Tmₛ (weakenTm' k b) ,
--    (⟦weakenTm⟧₁' k b bᵈ X= Y= b₁ ∙ starstar (⟦⟧Tmₛ a) (⟦⟧Tm₀ a ∙ ! (qq^₁ k X=)) (ap ft (⟦⟧Ty-ft B) ∙ ⟦⟧Ty-ft A ∙ ! (qq^₁ k X=)) refl ∙ ap2-irr star (⟦weakenTm⟧=' k a aᵈ X= Y=) (⟦weakenTy+⟧=' k B Bᵈ X= (⟦⟧Ty-ft A) (! (⟦weakenTy⟧=' k A Aᵈ X= Y=) ∙ ap2-irr star (qq^=p (! (⟦⟧Ty-ft A))) refl))) , tt)
-- ⟦weakenTm⟧ᵈ' k (pr1 A B u) (Aᵈ , Bᵈ , uᵈ , uₛ , u₁ , tt) X= Y= =
--   (⟦weakenTy⟧ᵈ' k A Aᵈ X= Y= ,
--    ⟦weakenTy⟧ᵈ' (prev k) B Bᵈ (X= ∙ ap (ft^ k) (! (⟦⟧Ty-ft A))) (! (⟦weakenTy⟧=' k A Aᵈ X= Y=) ∙ ap2-irr star (qq^=p {k = k} (! (⟦⟧Ty-ft A))) refl) ,
--    ⟦weakenTm⟧ᵈ' k u uᵈ X= Y= ,
--    ⟦⟧Tmₛ (weakenTm' k u) ,
--    (⟦weakenTm⟧₁' k u uᵈ X= Y= u₁ ∙ SigStrNat (qq^ k X=) (ap ft (⟦⟧Ty-ft B) ∙ ⟦⟧Ty-ft A ∙ ! (qq^₁ k X=)) ∙ ap SigStr (ap2-irr star (ap2-irr qq (qq^=p (! (⟦⟧Ty-ft A))) (⟦⟧Ty-ft B)) refl ∙ ⟦weakenTy⟧='prev k B Bᵈ (X= ∙ ap (ft^ k) (! (⟦⟧Ty-ft A))) refl (ap2-irr star (qq^=p (⟦⟧Ty-ft A)) refl ∙ ⟦weakenTy⟧=' k A Aᵈ X= Y=))) , tt)
...
-- {- WIP
-- ⟦weakenTm⟧ᵈ' k (natelim P dO dS u) (Pᵈ , P= , dOᵈ , dOₛ , dO₁ , dSᵈ , dSₛ , dS₁ , uᵈ , uₛ , u₁ , tt) X= refl =
--   (a1 ,
--    a2 ,
--    a3 ,
--    a4 ,
...
--       wP= : _
--       wP= = ⟦weakenTy⟧=' (prev k) P Pᵈ (X= ∙ ap (ft^ k) (! NatStr=)) (! naturalityNat)
...
--       a5 = ⟦weakenTm⟧₁' k dO dOᵈ X= refl dO₁ ∙ {!starqq^-term _ _ _ zeroStrₛ (zeroStr₀ _) _!} ∙ ap2-irr star (zeroStrNat _ (! (qq^₁ k X=)) ∙ ap zeroStr (qq^₀ k X=)) (ap2-irr star (qq^=p (⟦⟧Ty-ft P)) refl ∙ wP=)

--       a6 : isDefined
--              (⟦ weakenTm' (prev (prev k)) dS ⟧Tm
--               (⟦ weakenTy' (prev k) P ⟧Ty (NatStr (star^ k _ _ X=)) $ a1))
--       a6 = {!⟦weakenTm⟧ᵈ' (prev (prev k)) dS dSᵈ (X= ∙ ap (ft^ k) (! (ap ft (⟦⟧Ty-ft P) ∙ NatStr=))) (! wP= ∙ ap2-irr star (ap2-irr qq (qq^=p NatStr= ∙ ! (qq^=p (ap ft (⟦⟧Ty-ft P) ∙ NatStr=))) (! (⟦⟧Ty-ft P))) refl)!}
...
-- -- ⟦⟧TmEq r {u = natelim P dO dS u} (NatelimCong dP dP= ddO= ddS= du=) = {!!}

-- -- ⟦⟧TmEq r (IdUUCong da= du= dv=) = ap-irr-idStr (⟦⟧TmEq r da=) (⟦⟧TmEq r du=) (⟦⟧TmEq r dv=)
-- -- ⟦⟧TmEq r (ReflCong dA= du=) = ap-irr-reflStr (⟦⟧TyEq r dA=) (⟦⟧TmEq r du=)
-- -- --⟦⟧TmEq r {u = jj A P d a b p} (JjCong dA dP dd da db dp) = ?
