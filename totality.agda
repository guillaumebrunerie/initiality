...
⟦⟧Tmᵈ {Γ = Γ} {X = X} r {u = jj A P d a b p} (JJ dA dP dd da db dp) =
  let Aᵈ = ⟦⟧Tyᵈ r dA in
  let Pᵈ : isDefined (⟦ P ⟧Ty (IdStr (star (pp (star (pp (⟦ A ⟧Ty X $ Aᵈ)) (⟦ A ⟧Ty X $ Aᵈ) pp₁)) (star (pp (⟦ A ⟧Ty X $ Aᵈ)) (⟦ A ⟧Ty X $ ⟦⟧Tyᵈ r dA) pp₁) pp₁) (ss (pp (star (pp (⟦ A ⟧Ty X $ Aᵈ)) (⟦ A ⟧Ty X $ Aᵈ) pp₁))) ssₛ (ss₁' (pp₁ ∙ ft-star ∙ pp₀) ∙ star-comp pp₁) (ss (idC (star (pp (⟦ A ⟧Ty X $ Aᵈ)) (⟦ A ⟧Ty X $ Aᵈ) pp₁))) ssₛ (ss₁' id₁ ∙ ap2-irr star (id-left' pp₀) refl)))
      Pᵈ = cong⟦⟧Ty {X = {!!}} (ap-irr-IdStr (! (⟦weakenTy⟧=' last (weakenTy A) (⟦weakenTy⟧ᵈ' last A Aᵈ (⟦⟧Ty-ft A) refl) (⟦⟧Ty-ft (weakenTy A)) refl) ∙ ap2-irr star (ap pp (! (⟦weakenTy⟧=' last A Aᵈ (⟦⟧Ty-ft A) refl))) (! (⟦weakenTy⟧=' last A Aᵈ (⟦⟧Ty-ft A) refl))) (ap-irr (λ z p → ⟦ var (prev last) ⟧Tm z $ p) (! (⟦weakenTy⟧=' last A Aᵈ (⟦⟧Ty-ft A) refl)) ∙ ap ss (ap2-irr comp (ap ss (ap idC (ft-star ∙ pp₀))) refl) ∙ ss-comp {g = pp (⟦ A ⟧Ty X $ ⟦⟧Tyᵈ r dA)} ∙ ap ss (! assoc ∙ ap2-irr comp (ap2-irr comp (ap2-irr qq (! (id-left' (pp₀ ∙ id₁) ∙ ap pp id₁)) refl) refl ∙ ! ss-qq) refl ∙ id-right' (pp₁ ∙ ft-star ∙ pp₀))) ((ap-irr (λ z p → ⟦ var last ⟧Tm z $ p) (! (⟦weakenTy⟧=' last A Aᵈ (⟦⟧Ty-ft A) refl))))) (⟦⟧Tyᵈ {Γ = ((Γ , A) , weakenTy A) , id (weakenTy (weakenTy A)) (var (prev last)) (var last)} (respectsCtxExt {Γ = (Γ , A) , weakenTy A} (respectsCtxExt {Γ = Γ , A} (respectsCtxExt {Γ = Γ} r A {Aᵈ = Aᵈ}) (weakenTy A) {Aᵈ = ⟦weakenTy⟧ᵈ' last A Aᵈ (⟦⟧Ty-ft A) refl}) (id (weakenTy (weakenTy A)) (var (prev last)) (var last)) {Aᵈ = (⟦weakenTy⟧ᵈ' last (weakenTy A) (⟦weakenTy⟧ᵈ' last A Aᵈ (⟦⟧Ty-ft A) refl) (⟦⟧Ty-ft (weakenTy A)) refl ,
                                  (tt , (ss₀ ∙ id₀) , tt) ,
                                  ssₛ ,
                                  ...
                                  tt ,
                                  ssₛ ,
                                  (ss₁' id₁ ∙ ap2-irr star (id-left' pp₀) refl ∙ ⟦weakenTy⟧=' last (weakenTy A) (⟦weakenTy⟧ᵈ' last A Aᵈ (⟦⟧Ty-ft A) refl) (⟦⟧Ty-ft (weakenTy A)) refl) , tt)}) {A = P} dP) in
   (Aᵈ ,
    Pᵈ ,
    ⟦⟧Ty-ft P ,
    ⟦⟧Tmᵈ (respectsCtxExt r A) dd ,
    ⟦⟧Tmₛ d ,
    (⟦⟧Tm₁ (respectsCtxExt r A) dd ∙ ! (⟦tsubst⟧Ty= P Pᵈ ((idMor _ , var last) , refl (weakenTy A) (var last)) {!!}) ∙ ap2-irr star (ap2-irr comp (ap2-irr qq (⟦idMor+⟧= (ap ft (IdStr= ∙ ft-star ∙ pp₀ ∙ ⟦weakenTy⟧=' last A Aᵈ (⟦⟧Ty-ft A) refl) ∙ ⟦⟧Ty-ft (weakenTy A)) (var last) tt  {!ss₁' id₁ ∙ ap2-irr star (id-left' pp₀) refl ∙ ! (IdStr= ∙ ft-star ∙ pp₀)!}) refl) (ap-irr-reflStr (! (⟦weakenTy⟧=' last A Aᵈ (⟦⟧Ty-ft A) refl)) refl)) refl ∙ {!!} ),
    ⟦⟧Tmᵈ r da ,
    ⟦⟧Tmₛ a ,
    ⟦⟧Tm₁ r da ,
    ⟦⟧Tmᵈ r db ,
    ...
    ⟦⟧Tmₛ p ,
    ⟦⟧Tm₁ r dp , tt)
    {- (ap2-irr comp (ap2-irr qq (⟦idMor+⟧= (ap ft (ft-star ∙ pp₀ ∙ ⟦weakenTy⟧=' last A Aᵈ (⟦⟧Ty-ft A) refl) ∙ ⟦⟧Ty-ft (weakenTy A)) (var last) tt (ss₁' id₁ ∙ {!? ∙ !} ∙ ! (ft-star ∙ pp₀))) refl) (ap-irr-reflStr (! (⟦weakenTy⟧=' last A Aᵈ (⟦⟧Ty-ft A) refl)) refl))
...
-- ⟦⟧Tm₁ r {u = jj A P d a b p} (JJ dA dP dd da db dp) = ?
