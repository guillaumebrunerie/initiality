...
record StructuredCCatMor (sC sD : StructuredCCat) : Set where
  ...
  field
    ...
    emptyStr→ : (i : ℕ) (Γ : Ob C n) → Mor→ (emptyStr sC i Γ) ≡ emptyStr sD i (Ob→ Γ)
    emptyelimStr→ : (Γ : Ob C n) (A : Ob C (suc (suc n))) (A= : ft C A ≡ EmptyStr sC Γ) (u : Mor C n (suc n)) (uₛ : is-section C u) (u₁ : ∂₁ C u ≡ EmptyStr sC Γ)
                  → Mor→ (emptyelimStr sC Γ A A= u uₛ u₁) ≡ emptyelimStr sD (Ob→ Γ) (Ob→ A) (! ft→ ∙ ap Ob→ A= ∙ EmptyStr→ Γ) (Mor→ u) (Mor→ₛ uₛ) (Mor→₁ u₁ ∙ EmptyStr→ Γ )
    unitStr→ : (i : ℕ) (Γ : Ob C n) → Mor→ (unitStr sC i Γ) ≡ unitStr sD i (Ob→ Γ)
    ttStr→ : (Γ : Ob C n) → Mor→ (ttStr sC Γ) ≡ ttStr sD (Ob→ Γ)
    unitelimStr→ : (Γ : Ob C n) (A : Ob C (suc (suc n))) (A= : ft C A ≡ UnitStr sC Γ) (dtt : Mor C n (suc n)) (dttₛ : is-section C dtt) (dtt₁ : ∂₁ C dtt ≡ star C (ttStr sC Γ) A A= (ttStr₁ sC)) (u : Mor C n (suc n)) (uₛ : is-section C u) (u₁ : ∂₁ C u ≡ UnitStr sC Γ) → Mor→ (unitelimStr sC Γ A A= dtt dttₛ dtt₁ u uₛ u₁) ≡ unitelimStr sD (Ob→ Γ) (Ob→ A) (! ft→ ∙ ap Ob→ A= ∙ UnitStr→ Γ) (Mor→ dtt) (Mor→ₛ dttₛ) (Mor→₁ dtt₁ ∙ star→ ∙ ap-irr-star D (ttStr→ Γ) refl) (Mor→ u) (Mor→ₛ uₛ) (Mor→₁ u₁ ∙ UnitStr→ Γ)
    natStr→ : (i : ℕ) (Γ : Ob C n)
