...
abstract
  ...
  PathOver-Cst {p = refl} refl = reflo
  ...
  PathOver-PropPi : ∀ {l l' l''} {A : Set l} {B : A → Prop l'} {C : (a : A) → B a → Set l''}
                    {a a' : A} {p : a ≡ a'} {u : (b : B a) → C a b} {u' : (b' : B a') → C a' b'}
                    → ((y : B a) (y' : B a') → PathOver (uncurrify C) (Σ= {b = y} {b' = y'} p) (u y) (u' y'))
                    → PathOver (λ x → ((y : B x) → C x y)) p u u'
  PathOver-PropPi {p = refl} f = PathOver-refl-to (funextP (λ x → PathOver-refl-from (f x x)))
  
  PathOver-out : ∀ {l l'} {A : Set l} {B : A → Set l'} {a a' : A} {p : a ≡ a'} {u : B a} {u' : B a'}
               → PathOver B p u u' → PathOver (λ X → X) (ap B p) u u'
  PathOver-out reflo = reflo
  
  PathOver-in : ∀ {l l'} {A : Set l} {B : A → Set l'} {a a' : A} {p : a ≡ a'} {u : B a} {u' : B a'}
              → PathOver (λ X → X) (ap B p) u u' → PathOver B p u u'
  PathOver-in {p = refl} reflo = reflo
...
module _ {A : Set} {R : EquivRel A} where
  ...
  //-elim-PiP3 : ∀ {l l'} {B : A // R → Prop l} {C : (x : A // R) → B x → Set l'} (proj* : (a : A) (b : B (proj a)) → C (proj a) b) (eq* : {a a' : A} (r : a ≃ a') (y : B (proj a)) (y' : B (proj a')) → PathOver (uncurrify C) (Σ= (eq r)) (proj* a y) (proj* a' y')) → (x : A // R) → (y : B x) → C x y
