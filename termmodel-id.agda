...
jjStrS-eq {Γ = Γ} {Γ'} rΓ {A = A} {A'} rA A= A'= {P} {P'} rP P= P'= {d} {d'} rd dₛ d'ₛ d₁ d'₁ {a} {a'} ra aₛ a'ₛ a₁ a'₁ {b} {b'} rb bₛ b'ₛ b₁ b'₁ {p} {p'} rp pₛ p'ₛ p₁ p'₁ =
  dmorTm= dmorTmₛ dmorTmₛ rΓ (SubstTyMorEq2 (der Γ) (((der Γ , dTy A A=) , WeakTy (dTy A A=)) , Id (WeakTy (WeakTy (dTy A A=))) (VarPrev (WeakTy (dTy A A=)) (VarLast (dTy A A=))) (VarLast (WeakTy (dTy A A=)))) dP= ((idMor+= (der Γ) (dTm= rA A= ra aₛ a'ₛ a₁ a'₁) , congTmEqTy (! (weakenSubstTy (getTy A) (getTm a))) (dTm= rA A= rb bₛ b'ₛ b₁ b'₁)) , congTmEqTy (! (ap-id-Ty subst2Ty-weakenTy refl refl)) (dTm= (box (unOb≃ rΓ ,, IdCong (dTy= rA A=) (dTm= rA A= ra aₛ a'ₛ a₁ a'₁) (dTm= rA A= rb bₛ b'ₛ b₁ b'₁))) (IdStr=S (proj Γ) (proj A) A= (proj a) aₛ a₁ (proj b) bₛ  b₁) rp pₛ p'ₛ p₁ p'₁)))
