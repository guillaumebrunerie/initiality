...
emptyStrS-eq : (i : ℕ) {Γ Γ' : DCtx n} (rΓ : Γ ≃ Γ') → proj {R = MorEquiv} (emptyStrS-// i Γ) ≡ proj (emptyStrS-// i Γ')
emptyStrS-eq i rΓ = dmorTm= rΓ UU UU UUCong EmptyUU EmptyUU EmptyUUCong
...
emptyStrₛS  i = //-elimP (λ Γ → dmorTmₛ UU EmptyUU)
...
emptyelimStrS-eq : {Γ Γ' : DCtx n} (rΓ : Γ ≃ Γ') {A A' : DCtx (suc (suc n))} (rA : A ≃ A') (A= : ftS (proj A) ≡ EmptyStrS (proj Γ)) (A'= : ftS (proj A') ≡ EmptyStrS (proj Γ')) {u u' : DMor n (suc n)} (ru : u ≃ u') (uₛ : S.is-section (proj u)) (u'ₛ : S.is-section (proj u')) (u₁ : S.∂₁ (proj u) ≡ EmptyStrS (proj Γ)) (u'₁ : S.∂₁ (proj u') ≡ EmptyStrS (proj Γ')) → proj {R = MorEquiv} (emptyelimStrS-// Γ A A= u uₛ u₁) ≡ proj (emptyelimStrS-// Γ' A' A'= u' u'ₛ u'₁)
emptyelimStrS-eq {Γ = Γ} {Γ'} rΓ rA A= A'= ru uₛ u'ₛ u₁ u'₁ = eq (box (unOb≃ rΓ) (unOb≃ rΓ ,, SubstTyMorEq2 (der Γ) (der Γ , Empty) (dTy= rA A=) (idMor+= (der Γ) (dTm= (box (unOb≃ rΓ ,, EmptyCong)) refl ru uₛ u'ₛ u₁ u'₁)))
                                                                                 (idMor+= (der Γ) (EmptyelimCong (dTy= rA A=) (dTm= (box (unOb≃ rΓ ,, EmptyCong)) refl ru uₛ u'ₛ u₁ u'₁))))
...
emptyelimStrₛS = //-elimP (λ Γ → //-elimP (λ A A= → //-elimP (λ u uₛ u₁ → dmorTmₛ (SubstTy (dTy A A=) (idMor+ (der Γ) (dTm refl u uₛ u₁))) (Emptyelim (dTy A A=) (dTm refl u uₛ u₁)))))
