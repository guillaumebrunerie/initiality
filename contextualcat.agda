...
record CCat : Set₁ where
  ...
  -- ss-comp-section₁ g f g₀ f₁ gₛ p g₁ = ss₁ ∙ ap3-irr2 star {!gₛ!} {!!} {!!} --ss₁ ∙ ap3-irr2 star (! (assoc {q = ! (pp₀ ∙ comp₁)}) ∙ ap2-irr comp (ap2-irr comp (ap pp comp₁) refl ∙ fₛ ∙ ap id (! p)) refl ∙ id-right ) comp₁
    ...
    ss-of-section u uₛ = ! (ss-qq ∙ (ap-irr-comp (ap-irr-qq (uₛ ∙ ap id (is-section₀ uₛ refl)) refl {q' = refl} ∙ qq-id) refl) ∙ id-right (ss₁ refl ∙ ap-irr-star (uₛ ∙ ap id (is-section₀ uₛ refl)) refl {q' = refl} ∙ star-id)) --! (ss-qq ∙ ap-irr-comp (ap-irr-qq (uₛ ∙ ap id (is-section₀ uₛ refl)) (! (ss₁ refl ∙ (ap-irr-star (uₛ ∙ ap id (is-section₀ uₛ refl)) refl {q' = refl} ∙ star-id))) {q' = {!!}} ∙ qq-id) refl {f₁' = refl} ∙ id-right) -- ! (ss-qq ∙ ap2-irr comp (ap2-irr qq uₛ refl {b' = id₁ ∙ is-section₀ uₛ refl} ∙ ap2-irr qq (ap id (! (ap ft ss₁ ∙ ft-star ∙ comp₀))) (! (ss₁ ∙ ap2-irr star (uₛ ∙ ap id (is-section₀ uₛ refl)) refl ∙ star-id)) ∙ qq-id) refl ∙ id-right)
...
--   ss-pp' refl refl = ss-pp
  ...
  starTm++ g {X} {X'} X= {X''} X'= u u₀ g₁ = starTm+ (qq' g X' X'= g₁) X= u u₀ qq₁ --ss (comp' u (qq' (qq' g (ft (∂₀ u)) g₁) (∂₀ u) qq₁) qq₁)
  ...
  star-pp' {g = g} A= {B=} gₛ {g₁} = ! star-comp ∙ ap-irr-star (is-section= A= gₛ g₁) refl {q' = B=} ∙ star-id -- ! (star-comp {p = w2 ∙ ft-star} _) ∙ ap2-irr star (ap2-irr comp (ap pp (! (w2 ∙ ft-star ∙ pp₀))) refl ∙ gₛ ∙ ap id (is-section₀ gₛ (w2 ∙ ft-star ∙ pp₀) ∙ ! pp₁ ∙ w1)) refl ∙ star-id ∙ refl
...
--     ∙ star-comp _
  ...
  varCL₁ = ss₁ id₁ ∙ ap-irr-star (id-left pp₀) refl --ap2-irr star (id-left' pp₀) refl
  ...
  varC+₁ last refl refl = ss₁ pp₁ ∙ star-comp ∙ ap-irr-star refl (! varCL₁) --ap-irr-star refl (! varCL₁) refl --ss₁ ∙ star-comp pp₁ ∙ ? --ap2-irr star refl (! varCL₁)
  varC+₁ (prev k) {X = X} {Y = Y}  refl refl = ss₁ (comp₁ ∙ pp^₁ (prev k) Y) ∙ ap-irr-star (! assoc) refl ∙ star-comp ∙ (ap-irr-star refl (! (ss₁ (pp^₁ (prev k) Y)))) --ap-irr-star refl (! ss₁) refl -- ap2-irr star (! (assoc {q = pp^₁ (prev k) (ft X) ∙ ! pp₀})) refl ∙ star-comp (comp₁ ∙ pp₁) ∙ ap2-irr star refl (ap2-irr star (ap2-irr comp (ap pp (! (pp^₁ (prev k) _))) refl) (! (pp^₁ (prev k) _)) ∙ ! ss₁)
  
  ss-id₁ : {X : Ob (suc n)} {Y : Ob n} {X= : ft X ≡ Y} → ∂₁ (ss (id X)) ≡ star (pp X) X X= (pp₁ ∙ X=)
  ss-id₁ = ss₁ id₁ ∙ ap-irr-star (id-left pp₀) refl --ss₁' id₁ ∙ ap2-irr star (id-left' pp₀) refl
...
record CCatwithUUNat (ccat : CCat) (ccatuu : CCatwithUU ccat) : Set₁ where
  open CCat ccat renaming (Mor to MorC)
...
record CCatwithEl (ccat : CCat) (ccatuu : CCatwithUU ccat) (ccatuunat : CCatwithUUNat ccat ccatuu) : Set₁ where
  open CCat ccat renaming (Mor to MorC)
  open CCatwithUU ccatuu
...
ElNatRHS : {ccat : CCat} {ccatuu : CCatwithUU ccat} {ccatuunat : CCatwithUUNat ccat ccatuu} (ccatel : CCatwithEl ccat ccatuu ccatuunat) (let module S = CCat ccat) (let module SUU = CCatwithUU ccatuu) {i : ℕ} (g : S.Mor n m) {Δ : S.Ob n} (g₀ : S.∂₀ g ≡ Δ) {Γ : S.Ob m} (v : S.Mor m (suc m)) (vₛ : S.is-section v) (v₁ : S.∂₁ v ≡ SUU.UUStr i Γ) (g₁ : S.∂₁ g ≡ Γ) → S.Ob (suc n)
ElNatRHS {ccat = ccat} {ccatuu = ccatuu} {ccatuunat = ccatuunat} ccatel {i = i} g {Δ = Δ} g₀ v vₛ v₁ g₁ =
  let module S = CCat ccat
      module SUU = CCatwithUU ccatuu
      module SUUNat = CCatwithUUNat ccatuunat
      module SEl = CCatwithEl ccatel
  in
  SEl.ElStr i Δ (S.starTm g v (S.is-section₀ vₛ v₁ ∙ SUU.UUStr=) g₁) S.ssₛ (S.starTm₁ g SUU.UUStr= v vₛ v₁ g₁ ∙ SUUNat.UUStrNat g₀)

record CCatwithElNat (ccat : CCat) (ccatuu : CCatwithUU ccat) (ccatuunat : CCatwithUUNat ccat ccatuu)  (ccatel : CCatwithEl ccat ccatuu ccatuunat): Set₁ where
    open CCat ccat renaming (Mor to MorC)
    open CCatwithUU ccatuu
    ...
    field
      ElStrNat : {i : ℕ} {n m : ℕ} {g : MorC n m} {Δ : Ob n} (g₀ : ∂₀ g ≡ Δ) {Γ : Ob m} {v : MorC m (suc m)} {vₛ : is-section v} {v₁ : ∂₁ v ≡ UUStr i Γ} {g₁ : ∂₁ g ≡ Γ}
             → star g (ElStr i Γ v vₛ v₁) ElStr= g₁ ≡ ElNatRHS ccatel g g₀ v vₛ v₁ g₁
...
PiNatRHS : {ccat : CCat} (ccatpi : CCatwithPi ccat) (let module S = CCat ccat) (g : S.Mor n m) {Δ : S.Ob n} (g₀ : S.∂₀ g ≡ Δ) {Γ : S.Ob m} {A : S.Ob (suc m)} (A= : S.ft A ≡ Γ) (B : S.Ob (suc (suc m))) (B= : S.ft B ≡ A) (g₁ : S.∂₁ g ≡ Γ) → S.Ob (suc n)
PiNatRHS {ccat = ccat} ccatpi g {Δ = Δ} g₀ {A = A} A= B B= g₁ =
  let module S = CCat ccat
      module SPi = CCatwithPi ccatpi
  in
  SPi.PiStr Δ (S.star g A A= g₁) (S.ft-star ∙ g₀) (S.star+ g B B= A= g₁) (S.ft-star ∙ S.qq₀)

record CCatwithPiNat (ccat : CCat) (ccatpi : CCatwithPi ccat) : Set₁ where
  open CCat ccat renaming (Mor to MorC)
  open CCatwithPi ccatpi

  field
    PiStrNat : {g : MorC n m} {Δ : Ob n} (g₀ : ∂₀ g ≡ Δ) {Γ : Ob m} {A : Ob (suc m)} {A= : ft A ≡ Γ} {B : Ob (suc (suc m))} {B= : ft B ≡ A} {g₁ : ∂₁ g ≡ Γ}
             → star g (PiStr Γ A A= B B=) PiStr= g₁ ≡ PiNatRHS ccatpi g g₀ A= B B= g₁
...
record CCatwithNatNat (ccat : CCat) (ccatnat : CCatwithNat ccat) : Set₁ where
  open CCat ccat renaming (Mor to MorC)
...
record CCatwithapp (ccat : CCat) (ccatpi : CCatwithPi ccat) (ccatpinat : CCatwithPiNat ccat ccatpi) : Set₁ where
  open CCat ccat renaming (Mor to MorC)
  open CCatwithPi ccatpi
  open CCatwithPiNat ccatpinat
...
appStr₀ : {ccat : CCat} {ccatpi : CCatwithPi ccat} {ccatpinat : CCatwithPiNat ccat ccatpi} (ccatapp : CCatwithapp ccat ccatpi ccatpinat) (let module S = CCat ccat) (let module SPi = CCatwithPi ccatpi) (let module Sapp = CCatwithapp ccatapp) {Γ : S.Ob n} {A : S.Ob (suc n)} {A= : S.ft A ≡ Γ}  {B : S.Ob (suc (suc n))} {B= : S.ft B ≡ A} {f : S.Mor n (suc n)} {fₛ : S.is-section f} {f₁ : S.∂₁ f ≡ SPi.PiStr Γ A A= B B=} {a : S.Mor n (suc n)} {aₛ : S.is-section a} {a₁ : S.∂₁ a ≡ A} → S.∂₀ (Sapp.appStr Γ A A= B B= f fₛ f₁ a aₛ a₁) ≡ Γ
appStr₀ {ccat = ccat} ccatapp {A= = A=} {aₛ = aₛ} {a₁ = a₁} =
  let module S = CCat ccat
      module Sapp = CCatwithapp ccatapp
  in
    S.is-section₀ Sapp.appStrₛ Sapp.appStr₁ ∙ S.ft-star ∙ S.is-section₀ aₛ a₁ ∙ A=

appNatRHS : {ccat : CCat} {ccatpi : CCatwithPi ccat} {ccatpinat : CCatwithPiNat ccat ccatpi} (ccatapp : CCatwithapp ccat ccatpi ccatpinat) (let module S = CCat ccat) (let module SPi = CCatwithPi ccatpi) (g : S.Mor n m) {Δ : S.Ob n} (g₀ : S.∂₀ g ≡ Δ) {Γ : S.Ob m} {A : S.Ob (suc m)} (A= : S.ft A ≡ Γ) (B : S.Ob (suc (suc m))) (B= : S.ft B ≡ A)
                  (f : S.Mor m (suc m)) (fₛ : S.is-section f) (f₁ : S.∂₁ f ≡ SPi.PiStr Γ A A= B B=)
                  (a : S.Mor m (suc m)) (aₛ : S.is-section a) (a₁ : S.∂₁ a ≡ A) (g₁ : S.∂₁ g ≡ Γ) → S.Mor n (suc n)
appNatRHS {ccat = ccat} {ccatpi} {ccatpinat} ccatapp g {Δ = Δ} g₀ {A = A} A= B B= f fₛ f₁ a aₛ a₁ g₁ =
  let module S = CCat ccat
      module SPi = CCatwithPi ccatpi
      module SPiNat = CCatwithPiNat ccatpinat
      module Sapp = CCatwithapp ccatapp
      a₀ = S.is-section₀ aₛ a₁ ∙ A=
      f₀ = S.is-section₀ fₛ f₁ ∙ SPi.PiStr=
  in
    Sapp.appStr Δ (S.star g A A= g₁)
                  (S.ft-star ∙ g₀)
                  (S.star+ g B B= A= g₁)
                  (S.ft-star ∙ S.qq₀)
                  (S.starTm g f f₀ g₁) S.ssₛ (S.starTm₁ g SPi.PiStr= f fₛ f₁ g₁ ∙ SPiNat.PiStrNat g₀)
                  (S.starTm g a a₀ g₁) S.ssₛ (S.starTm₁ g A= a aₛ a₁ g₁)
                           
 
record CCatwithappNat (ccat : CCat) (ccatpi : CCatwithPi ccat) (ccatpinat : CCatwithPiNat ccat ccatpi) (ccatapp : CCatwithapp ccat ccatpi ccatpinat) : Set₁ where
  open CCat ccat renaming (Mor to MorC)
  open CCatwithPi ccatpi
  open CCatwithPiNat ccatpinat
  ...
  field
    appStrNat : {g : MorC n m} {Δ : Ob n} (g₀ : ∂₀ g ≡ Δ) {Γ : Ob m} {A : Ob (suc m)} {A= : ft A ≡ Γ} {B : Ob (suc (suc m))} {B= : ft B ≡ A} {f : MorC m (suc m)} {fₛ : is-section f} {f₁ : ∂₁ f ≡ PiStr Γ A A= B B=}
             ...
             → starTm g (appStr Γ A A= B B= f fₛ f₁ a aₛ a₁) (appStr₀ ccatapp) g₁
                ≡ appNatRHS ccatapp g g₀ A= B B= f fₛ f₁ a aₛ a₁ g₁
...
record CCatwithsuc (ccat : CCat) (ccatnat : CCatwithNat ccat) (ccatnatnat : CCatwithNatNat ccat ccatnat) : Set₁ where
  open CCat ccat renaming (Mor to MorC)
  open CCatwithNat ccatnat
  open CCatwithNatNat ccatnatnat
...
T-dS₁ : {ccat : CCat} {ccatnat : CCatwithNat ccat} {ccatnatnat : CCatwithNatNat ccat ccatnat} (ccatsuc : CCatwithsuc ccat ccatnat ccatnatnat) (let module S = CCat ccat) (let module SNat = CCatwithNat ccatnat) (Γ : S.Ob n) (P : S.Ob (suc (suc n))) (P= : S.ft P ≡ SNat.NatStr Γ) → S.Ob (suc (suc (suc n)))
T-dS₁ {ccat = ccat} {ccatnat = ccatnat} {ccatnatnat} ccatsuc  Γ P P= =
  let module S = CCat ccat
      module SNat = CCatwithNat ccatnat
      module SNatNat = CCatwithNatNat ccatnatnat
      module Ssuc = CCatwithsuc ccatsuc
  in
    S.star' (S.pp P)
            (S.star' (Ssuc.sucStr (SNat.NatStr Γ) (S.varC last (SNat.NatStr Γ)) (S.varCₛ last (SNat.NatStr Γ)) (S.varCL₁ ∙ SNatNat.NatStrNat S.pp₀))
                     (S.star' (S.qq' (S.pp (SNat.NatStr Γ)) (SNat.NatStr Γ) SNat.NatStr= (S.pp₁ ∙ SNat.NatStr=))
                                                                                    P P= S.qq₁)
                     (S.ft-star ∙ S.qq₀ ∙ SNatNat.NatStrNat S.pp₀) Ssuc.sucStr₁) 
            (S.ft-star ∙ Ssuc.sucStr₀) (S.pp₁ ∙ P=) 


record CCatwithnatelim (ccat : CCat) (ccatnat : CCatwithNat ccat) (ccatnatnat : CCatwithNatNat ccat ccatnat) (ccatzero : CCatwithzero ccat ccatnat) (ccatsuc : CCatwithsuc ccat ccatnat ccatnatnat) : Set₁ where
  open CCat ccat renaming (Mor to MorC)
  open CCatwithNat ccatnat
  open CCatwithNatNat ccatnatnat
...
record StructuredCCat : Set₁ where
  field
    ...
    ccatUUNat : CCatwithUUNat ccat ccatUU
    ccatEl : CCatwithEl ccat ccatUU ccatUUNat
