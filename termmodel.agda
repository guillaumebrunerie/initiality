...
⊢==-is-prop ◇ ◇ tt tt = refl
⊢==-is-prop (Γ , A) (Γ' , A') (a , b) (a' , b') = ap-irr _,_ (⊢==-is-prop Γ Γ' a a')
...
⊢==∷>-is-prop : {n : ℕ} {Γ : Ctx n} {Δ : Ctx m} (δ δ' : Mor n m) → is-prop (Γ ⊢ δ == δ' ∷> Δ)
⊢==∷>-is-prop {Δ = ◇} ◇ ◇ tt tt = refl
⊢==∷>-is-prop {Δ = Δ , B} (δ , u) (δ' , u') (a , b) (a' , b') = ap-irr _,_ (⊢==∷>-is-prop δ δ' a a')

×-is-prop : {A B : Set} → is-prop A → is-prop B → is-prop (A × B)
×-is-prop pA pB (a , b) (a' , b') rewrite pA a a' | pB b b' = refl

×'-is-prop : {A B : Set} → is-prop A → is-prop B → is-prop (A ×' B)
×'-is-prop pA pB (a , b) (a' , b') rewrite pA a a' = refl

MorEquiv : (n m : ℕ) → EquivRel (DMor n m)
EquivRel._≃_ (MorEquiv n m) (((Γ , _), (Δ , _)), (δ , _)) (((Γ' , _), (Δ' , _)), (δ' , _)) = ((⊢ Γ == Γ') × (⊢ Δ == Δ')) ×' (Γ ⊢ δ == δ' ∷> Δ)
EquivRel._≃-is-prop_ (MorEquiv n m) (((Γ , _), (Δ , _)), (δ , _)) (((Γ' , _), (Δ' , _)), (δ' , _)) = ×'-is-prop (×-is-prop (⊢==-is-prop Γ Γ') (⊢==-is-prop Δ Δ')) (⊢==∷>-is-prop δ δ')
...
ptmor-uniqueS = //-elimPiIdR2 ptmor-uniqueS-//

.id-rightS-// : (f : DMor n m) → compS (idS m (∂₁S (proj f))) (proj f) (! (id₀S m (∂₁S (proj f)))) ≡ (proj f)
id-rightS-// {m = m} (((Γ , dΓ), (Δ , dΔ)), (δ , dδ)) = ap-irr (λ x z → proj (((Γ , dΓ), (Δ , dΔ)) , (x , z))) (idMor[]Mor δ) {b = SubstMor (idMorDerivable dΔ) dδ}
...
id-leftS-// {n = n} (((Γ , dΓ), (Δ , dΔ)), (δ , dδ)) = ap-irr (λ x z → proj (((Γ , dΓ), (Δ , dΔ)) , (x , z))) ([idMor]Mor δ) {b = SubstMor dδ (idMorDerivable dΓ)}
...
assocS-// (((Θ' , dΘ'), (Φ , dΦ)), (φ , dφ)) (((Δ' , dΔ'), (Θ , dΘ)), (θ , dθ)) (((Γ , dΓ), (Δ , dΔ)), (δ , dδ)) p q =
  ap-irr (λ x z → proj (((Γ , dΓ), (Φ , dΦ)) , (x , z)))
         ([]Mor-assoc δ θ φ)
         {b = aux-b}
         {b' = aux-b'}  where

  aux-b = SubstMor (SubstMor dφ (ConvMor dθ (CtxRefl dΔ') (reflect q))) (ConvMor dδ (CtxRefl dΓ) (reflect p))
  aux-b' = SubstMor dφ (SubstMor (ConvMor dθ (CtxRefl dΔ') (reflect q)) (ConvMor dδ (CtxRefl dΓ) (reflect p)))
...
ssS-//-u {m = m} f@(((Γ , dΓ), ((Δ , B), (dΔ , dB))), ((δ , u), (dδ , du))) = pair-DMor (idS-u m (Γ , dΓ)) (SubstTy dB dδ) (congTm (! ([idMor]Ty _)) du)
...
ssS-eq {m = m} f@(((Γ , dΓ), ((Δ , B), (dΔ , dB))), ((δ , u), (dδ , du))) f'@(((Γ' , dΓ'), ((Δ' , B'), (dΔ' , dB'))), ((δ' , u'), (dδ' , du'))) p@((dΓ= , (dΔ= , dB=)) , (dδ= , du=))
  = eq {a = ssS-//-u f} {b = ssS-//-u f'} ((dΓ= , (dΓ= , (SubstTySubstEq dB= dδ=))) , (MorRefl (idMorDerivable dΓ) , congTmEq (! ([idMor]Ty _)) du=))
...
ss₁S-// (((Γ , dΓ), ((Δ , B), (dΔ , dB))), ((δ , u), (dδ , du))) = ap-irr (λ x z → proj (x , z)) (ap (λ γ → (Γ , B [ γ ]Ty)) (! (weakenidMor[]Mor δ u))) {b' = dΓ , SubstTy dB (SubstMor (WeakMor (idMorDerivable dΔ)) (dδ , du))}
...
pp-qqS = //-elimPiId2 pp-qqS-//

star-idS : {n : ℕ} (X : ObS (suc n)) → starS (idS n (ftS X)) X (id₁S n (ftS X)) ≡ X
star-idS = //-elimId _ _ (λ {((Γ , A), (dΓ , dA)) → ap-irr (λ x z → proj (x , z)) (ap (λ z → Γ , z) ([idMor]Ty A)) {b = dΓ , (SubstTy dA (idMorDerivable dΓ))}})

qq-idS : (X : ObS (suc n)) → qqS (idS n (ftS X)) X (id₁S n (ftS X)) ≡ idS (suc n) X
qq-idS {n = n} = //-elimId _ _ (λ {((Γ , A), (dΓ , dA)) → ap-irr2 (λ x z t → (proj ((((Γ , x) , (dΓ , z)), ((Γ , A), (dΓ , dA))), ((weakenMor' last (idMor n) , var last) , t)))) ([idMor]Ty A) {b = SubstTy dA (idMorDerivable dΓ)} {b' = dA} {c = (WeakMor (idMorDerivable dΓ)) , (congTm (weaken[]Ty A (idMor n) last) (VarRule last (WeakTy (SubstTy dA (idMorDerivable dΓ)))))} {c' = (WeakMor (idMorDerivable dΓ)) , (congTm (ap weakenTy (! ([idMor]Ty A)) ∙ weaken[]Ty A (idMor n) last) (VarRule last (WeakTy dA)))}})
...
star-compS-// ((Δd@(Δ , dΔ) , Θd) , (θ , dθ)) ((Γd@(Γ , dΓ) , Δd'@(Δ' , dΔ')) , (δ , dδ)) ((Γ' , A), (dΓ' , dA)) p q =
  ap-irr (λ x z → proj ((Γ , x), z)) (! ([]Ty-assoc δ θ A))
         {b = aux-b}
         {b' = aux-b'}
    where
    .aux-b : Σ (⊢ Γ) (λ _ → Derivable (Γ ⊢ (A [ θ [ δ ]Mor ]Ty)))
    aux-b = dΓ , SubstTy dA (SubstMor (ConvMor dθ (CtxRefl dΔ) (reflect q)) (ConvMor dδ (CtxRefl dΓ) (reflect p)))

    .aux-b' : Σ (⊢ Γ) (λ _ → Derivable (Γ ⊢ ((A [ θ ]Ty) [ δ ]Ty)))
    aux-b' = dΓ , SubstTy (SubstTy dA (ConvMor dθ (CtxRefl dΔ) (reflect q))) (ConvMor dδ (CtxRefl dΓ) (reflect p))
...
star-compS = //-elimPiId3 star-compS-//
...
ss-compS-// ((Θ' , C), (dΘ' , dC)) (((Δ' , dΔ'), (Θ , dΘ)), (θ , dθ)) (((Γ , dΓ), ((Δ , B), (dΔ , dB))), ((δ , u), (dδ , du))) g₁ f₁ = {!!}
