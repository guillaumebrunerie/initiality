...
compS-// g f X g₀ f₁ = dmor (lhs f) (rhs g) (SubstMor (morDer g) (ConvMor (morDer f) (CtxRefl (der (lhs f))) (reflectOb (f₁ ∙ ! g₀))))
...
ppS-// Γ = dmor Γ (ftS-// Γ) (ConvMor (WeakMor (idMorDerivable (getdCtx Γ))) (CtxTy=Ctx' Γ) (CtxRefl (getdCtx Γ)))
...
pp₀S = //-elimP (λ {(dctx' _) → refl})

pp₁S : (X : ObS (suc n)) → ∂₁S (ppS X) ≡ ftS X
pp₁S = //-elimP (λ {(dctx' _) → refl})

ptS : ObS 0
ptS = proj (dctx {ctx = ◇} tt)

pt-uniqueS : (X : ObS 0) → X ≡ ptS
pt-uniqueS = //-elimP (λ {(dctx' {ctx = ◇} _) → eq (box tt)})

ptmorS : (X : ObS n) → MorS n 0
ptmorS = //-rec (λ Γ → proj (dmor Γ (dctx {ctx = ◇} tt) {mor = ◇} tt)) (λ r → eq (box (unOb≃ r) tt tt))
...
ptmor-uniqueS-// X (dmor' Γ (dctx' {ctx = ◇} tt) {mor = ◇} tt) p q = eq (box (reflectOb p) (reflectOb q) tt)
...
qqS-// f X Y q f₁ = dmor (starS-// f X Y q f₁) X (ConvMor (WeakMor+ (getdTy X) (ConvMor (morDer f) (CtxRefl (der (lhs f))) (reflectOb (f₁ ∙ ! q)))) (CtxRefl (der (lhs f) , SubstTy (getdTy X) (ConvMor (morDer f) (CtxRefl (der (lhs f))) (reflectOb (f₁ ∙ ! q))))) (CtxTy=Ctx' X))
...
ssS-// f = dmor (lhs f) (dctx (der (lhs f) , SubstTy (getdTy (rhs f)) (getdMor f))) (idMor+ (der (lhs f)) (getdTm f))

ssS-eq : (f f' : DMor m (suc n)) (rf : f ≃ f') → proj {R = MorEquiv} (ssS-// f) ≡ proj (ssS-// f')
ssS-eq f f' rf = eq (box (unMor≃-lhs rf) (unMor≃-lhs rf ,, SubstTyFullEq' (der (lhs f)) (getdCtx (rhs f)) (getTy= {Γ = rhs f} {Γ' = rhs f'} (box (unMor≃-rhs rf))) (getLHS= {Δ = rhs f} (unMor≃-mor rf))) (idMor+= (der (lhs f)) (getRHS= (unMor≃-mor rf))))
...
ss₁S-// f X f₁ = eq (box (CtxRefl (der (lhs f)) ,, SubstTyFullEq' (der (lhs f)) (getdCtx (rhs f)) (getTy= {Γ = rhs f} {Γ' = X}(box (reflectOb f₁))) (getLHS= {Δ = rhs f} (congMorEq refl refl refl (! (idMor[]Mor _)) (MorRefl (morDer f)))))) 
...
ss-compS-// U X p g g₁ f f₁ = eq (box (CtxRefl (der (lhs f))) (CtxRefl (der (lhs f)) ,, congTyEq refl (! (ap (λ z → (getTy U) [ z ]Ty) (weakenMorInsert' _ _) ∙ ! ([]Ty-assoc _ _ _))) (SubstTyEq (getTy= {Γ = rhs f} {Γ' = dctx (der (lhs g) , SubstTy (ConvTy (getdTy U) (reflectOb (p ∙ ! g₁))) (morDer g))} (box (reflectOb f₁))) (getdMor f))) (MorRefl (idMor+ (der (lhs f)) (getdTm f))))
...
S-is-section₀ : {u : DMor n (suc n)} {X : S.Ob (suc n)} (uₛ : S.is-section (proj u)) (u₁ : S.∂₁ (proj u) ≡ X) → proj (dctx (der (lhs u))) ≡ S.ft X
S-is-section₀ uₛ u₁ = ap proj eta  ∙ S.is-section₀ uₛ u₁  where
  eta : {Y : DCtx n} → dctx (der Y) ≡ Y
  eta {Y = dctx' derY} = refl


getMor=idMor' : {a : DMor n (suc n)} (aₛ : S.is-section (proj a)) → ctx (lhs a) ⊢ getMor a == idMor n ∷> getCtx (ctx (rhs a))
getMor=idMor' {a = dmor' _ (dctx' dΔ) {mor = (δ , u)} _} aₛ = congMorEq refl refl (weakenMorInsert _ _ u  ∙ idMor[]Mor _) refl ((unMor≃-mor (reflect (S.is-section= refl aₛ refl))))

morTm=idMorTm' : {a : DMor n (suc n)} (aₛ : S.is-section (proj a)) → ctx (lhs a) ⊢ mor a == idMor n , getRHS (mor a) ∷> (ctx (rhs a))
morTm=idMorTm' {a = dmor' _ (dctx' {ctx = _ , _} (_ , _)) {mor = (_ , _)} (_ , du)} aₛ = (getMor=idMor' aₛ) , (TmRefl du)


dTy≃ : {Γ : DCtx n} (A : DCtx (suc n)) (A= : (ftS-// A) ≃ Γ) → Derivable (ctx Γ ⊢ getTy A)
dTy≃ (dctx' {ctx = (_ , _ )} (_ , dA)) (box A=) = ConvTy dA A=

Ctx≃ft+Ty : {Γ : DCtx n} {A : DCtx (suc n)} (A= : ftS-// A ≃ Γ) →  A ≃ dctx {ctx = ctx Γ , getTy A} (der Γ , dTy≃ A A=)
Ctx≃ft+Ty {A = A@(dctx' {ctx = _ , _} _)} (box A=) = box (A= ,, TyRefl (getdTy A))

dTy : {Γ : DCtx n} (A : DCtx (suc n)) (A= : proj {R = ObEquiv} (dctx (getdCtx A)) ≡ proj Γ) → Derivable (ctx Γ ⊢ getTy A)
dTy (dctx' {ctx = (_ , _ )} (_ , dA)) dA= = ConvTy dA (reflectOb dA=)

dTy' : {Γ : DCtx n} (A : DCtx (suc n)) (A= : ⊢ ctx (ftS-// A) == ctx Γ) → Derivable (ctx Γ ⊢ getTy A)
dTy' (dctx' {ctx = (_ , _ )} (_ , dA)) dA= = ConvTy dA dA=


dTy= : {Γ : DCtx n} {A A' : DCtx (suc n)} (rA : A ≃ A') (A= : ftS (proj A) ≡ proj Γ) → Derivable (ctx Γ ⊢ getTy A == getTy A')
dTy= {A = dctx' {ctx = (ΓA , A)} (dΓA , dA)} {A' = dctx' {ctx = (ΓA' , A')} (dΓA' , dA')} (box (dΓ= , dA=)) A= = ConvTyEq dA= (CtxTran (CtxSymm dΓ=) (reflectOb A=))

dTy=≃ : {Γ : DCtx n} {A A' : DCtx (suc n)} (rA : A ≃ A') (A= : (ftS-// A) ≃ Γ) → Derivable (ctx Γ ⊢ getTy A == getTy A')
dTy=≃ {A = dctx' {ctx = (ΓA , A)} (dΓA , dA)} {A' = dctx' {ctx = (ΓA' , A')} (dΓA' , dA')} (box (dΓ= , dA=)) (box A=) = ConvTyEq dA= (CtxTran (CtxSymm dΓ=) A=)
...
combine' : {Γ : DCtx n} {A : DCtx (suc n)} (A= : ftS (proj A) ≡ proj Γ) (B : DCtx (suc (suc n))) (B= : ftS (proj B) ≡ proj A) → ⊢ ctx (ftS-// B) == (ctx Γ , getTy A)
combine' {A = A} A= B B= = CtxTran (reflectOb B=) (CtxTran (CtxSymm (CtxTy=Ctx' A)) (reflectOb A= ,, TyRefl (getdTy A)))
...
lemmathing {Γ = dctx' {ctx = Γ , A} (dΓ , dA)} {Δ = dctx' {ctx = Δ , B} (dΔ , dB)} (box (dΓ= , dA=)) = dA=
...
getMor=idMor≃ : {Γ : DCtx n} {A : DCtx (suc n)} (A= : (ftS-// A) ≃ Γ) (a : DMor n (suc n)) (aₛ : S.is-section (proj a)) (a₁ : (∂₁S-// a) ≃ A) → ctx Γ ⊢ getMor a == idMor n ∷> ctx Γ
getMor=idMor≃ A= a aₛ a₁ = ConvMorEq (getMor=idMor' aₛ) (unOb≃ (tra (reflect (S.is-section₀ aₛ (eq a₁))) A=)) (CtxTran (getCtx= (unOb≃ a₁)) (unOb≃ A=))
...
dTm' : {Γ : DCtx n} {A : DCtx (suc n)} (A= : ⊢ ctx (ftS-// A) == ctx Γ) (a : DMor n (suc n)) (aₛ : S.is-section (proj a)) (a₁ : ⊢ ctx (∂₁S-// a) == ctx A) → Derivable (ctx Γ ⊢ getTm a :> getTy A)
dTm' {A = A} A= a aₛ a₁ =
  let lhsa=Γ = CtxTran (reflectOb (S.is-section₀ aₛ (eq (box {Γ = ∂₁S-// a} {Γ' = A} a₁)))) A=
      lhsa=ftA = reflectOb (S.is-section₀ aₛ (eq (box {Γ = ∂₁S-// a} {Γ' = A} a₁)))     
  in
  ConvTm2 (getdTm a) lhsa=Γ (TyTran (ConvTy (getdTy (rhs a)) (CtxTran (getCtx= a₁) (CtxSymm (lhsa=ftA)))) (congTyEq refl ([idMor]Ty _) (SubstTyMorEq (getdTy (rhs a)) (getdMor a) (getMor=idMor' aₛ))) (ConvTyEq (lemmathing (box {Γ = ∂₁S-// a} {Γ' = A} a₁)) (CtxSymm lhsa=ftA)))
   
dTm≃ : {Γ : DCtx n} {A : DCtx (suc n)} (A= : ftS-// A ≃ Γ) (a : DMor n (suc n)) (aₛ : S.is-section (proj a)) (a₁ : ∂₁S-// a ≃ A) → Derivable (ctx Γ ⊢ getTm a :> getTy A)
dTm≃ {A = A} A= a aₛ a₁ =
  let lhsa=ftA = reflectOb (S.is-section₀ aₛ (eq a₁))
      lhsa=Γ = CtxTran lhsa=ftA (unOb≃ A=)
  in
  ConvTm2 (getdTm a) lhsa=Γ (TyTran (ConvTy (getdTy (rhs a)) (CtxTran (getCtx= (unOb≃ a₁)) (CtxSymm (lhsa=ftA)))) (congTyEq refl ([idMor]Ty _) (SubstTyMorEq (getdTy (rhs a)) (getdMor a) (getMor=idMor' aₛ))) (ConvTyEq (lemmathing a₁) (CtxSymm lhsa=ftA)))
...
dTm=≃ : {Γ : DCtx n} {A A' : DCtx (suc n)} (A= : (ftS-// A) ≃ Γ) {a a' : DMor n (suc n)} (ra : a ≃ a') (aₛ : S.is-section (proj a)) (a'ₛ : S.is-section (proj a')) (a₁ : (∂₁S-// a) ≃ A) (a'₁ : (∂₁S-// a') ≃ A') → Derivable (ctx Γ ⊢ getTm a == getTm a' :> getTy A)
dTm=≃ A= {a} {a'} ra aₛ a'ₛ a₁ a'₁ =
  let lhsa=ftA = reflectOb (S.is-section₀ aₛ (eq a₁))
      lhsa=Γ = CtxTran lhsa=ftA (unOb≃ A=)
      rhsa=A = unOb≃ a₁
    in
    ConvTmEq2 (getRHS= (unMor≃-mor ra)) lhsa=Γ (congTyEq refl ([idMor]Ty _) (SubstTyFullEq' (der (lhs a)) (getdCtx (rhs a)) (getTy= a₁) (getMor=idMor' aₛ)))

dTm= : {Γ : DCtx n} {A A' : DCtx (suc n)} (A= : ftS (proj A) ≡ proj Γ) {a a' : DMor n (suc n)} (ra : a ≃ a') (aₛ : S.is-section (proj a)) (a'ₛ : S.is-section (proj a')) (a₁ : ∂₁S (proj a) ≡ proj A) (a'₁ : ∂₁S (proj a') ≡ proj A') → Derivable (ctx Γ ⊢ getTm a == getTm a' :> getTy A)
dTm= A= {a} {a'} ra aₛ a'ₛ a₁ a'₁ = 
  let lhsa=Γ = reflectOb (S.is-section₀ aₛ a₁ ∙ A=)
      lhsa=ftA = reflectOb (S.is-section₀ aₛ a₁)
      rhsa=A = reflectOb a₁
...
dTm=' : {Γ : DCtx n} {A A' : DCtx (suc n)} (A= : ⊢ ctx (ftS-// A) == ctx Γ) {a a' : DMor n (suc n)} (ra : a ≃ a') (aₛ : S.is-section (proj a)) (a'ₛ : S.is-section (proj a')) (a₁ : ⊢ ctx (∂₁S-// a) == ctx A) (a'₁ : ⊢ ctx (∂₁S-// a') == ctx A') → Derivable (ctx Γ ⊢ getTm a == getTm a' :> getTy A)
dTm=' {A = A} A= {a} {a'} ra aₛ a'ₛ a₁ a'₁ = 
  let lhsa=Γ = CtxTran (reflectOb (S.is-section₀ aₛ (eq (box {Γ  = ∂₁S-// a} {Γ' = A} a₁)))) A=
      lhsa=ftA = reflectOb (S.is-section₀ aₛ (eq (box {Γ  = ∂₁S-// a} {Γ' = A} a₁)))
      rhsa=A = a₁
  in
  ConvTmEq2 (getRHS= (unMor≃-mor ra)) lhsa=Γ (TyTran (ConvTy (getdTy (rhs a)) (CtxTran (getCtx= a₁) (CtxSymm (lhsa=ftA)))) (congTyEq refl ([idMor]Ty _) (SubstTyMorEq (getdTy (rhs a)) (getdMor a) (getMor=idMor' aₛ))) (ConvTyEq (lemmathing (box {Γ = ∂₁S-// a} {Γ' = A} a₁)) (CtxSymm lhsa=ftA)))
...
dTmSubst= {A = A} A= {B} {B'} rB B= B'= {a} {a'} ra aₛ a'ₛ a₁ a'₁ {b} {b'} rb bₛ b'ₛ b₁ b'₁  = 
  let lhsa=Γ = reflectOb (S.is-section₀ aₛ a₁ ∙ A=)
      ...
      rhsa=A = reflectOb a₁
  in
    ConvEq (SubstTy (dTy B B=) (ConvMor (morDer a) lhsa=Γ rhsa=A)) (dTm= (S-is-section₀ aₛ a₁ ∙ A=) rb bₛ b'ₛ b₁ b'₁) (SubstTyMorEq (dTy B B=) (ConvMor (morDer a) lhsa=Γ rhsa=A) (ConvMorEq (morTm=idMorTm' aₛ) lhsa=Γ rhsa=A))


dTm+= : {Γ : DCtx n} {A : DCtx (suc n)} (A= : ftS (proj A) ≡ proj Γ) {B B' : DCtx (suc (suc n))} (B= : ftS (proj B) ≡ proj A) {u u' : DMor (suc n) (suc (suc n))} (ru : u ≃ u') (uₛ : S.is-section (proj u)) (u'ₛ : S.is-section (proj u')) (u₁ : ∂₁S (proj u) ≡ proj B) (u'₁ : ∂₁S (proj u') ≡ proj B') → Derivable ((ctx Γ , getTy A) ⊢ getTm u == getTm u' :> getTy B)
dTm+= A= {B = B} B= ru uₛ u'ₛ u₁ u'₁ = dTm= (combine A= B B=) ru uₛ u'ₛ u₁ u'₁

up-to-CtxTyEq : {Γ Δ : DCtx (suc n)} (CtxEq : ⊢ getCtx (ctx Γ) == getCtx (ctx Δ)) (TyEq : getTy Γ ≡ getTy Δ) → proj {R = ObEquiv} Γ ≡ proj Δ
up-to-CtxTyEq {Γ = dctx' {ctx = Γ , A} (dΓ , dA)} {Δ = dctx' {ctx = Δ , B} (dΔ , dB)} Γ= refl = eq (box (Γ= ,, TyRefl dA))

up-to-rhsTyEq : {δ δ' : DMor n (suc n)} → {{ctx (lhs δ) ≡ getCtx (ctx (rhs δ))}} →  {{ctx (lhs δ') ≡ getCtx (ctx (rhs δ'))}} → {{mor δ ≡ mor δ'}} → {{ctx (lhs δ) ≡ ctx (lhs δ')}} → (getTy (rhs δ) ≡ getTy (rhs δ')) → proj {R = MorEquiv} δ ≡ proj δ'
up-to-rhsTyEq {δ = dmor' (dctx' dΓ) (dctx' {ctx = Γ' , A} (dΓ' , dA)) morDer} {δ' = dmor' (dctx' dΔ) (dctx' {ctx = Δ' , B} (dΔ' , dB)) morDer'} {{refl}} {{refl}} {{refl}} {{refl}} refl = refl

up-to-rhsTyEq' : {δ δ' : DMor n (suc n)} → {{ctx (lhs δ) ≡ getCtx (ctx (rhs δ))}} →  {{ctx (lhs δ') ≡ getCtx (ctx (rhs δ'))}} → {{mor δ ≡ mor δ'}} → (⊢ ctx (lhs δ) == ctx (lhs δ')) → (getTy (rhs δ) ≡ getTy (rhs δ')) → proj {R = MorEquiv} δ ≡ proj δ'
up-to-rhsTyEq' {δ = dmor' (dctx' dΓ) (dctx' {ctx = Γ' , A} (dΓ' , dA)) morDer} {δ' = dmor' (dctx' dΔ) (dctx' {ctx = Δ' , B} (dΔ' , dB)) morDer'} {{refl}} {{refl}} {{refl}} Γ= refl = eq (box Γ= (Γ= ,, TyRefl dA) (MorRefl morDer))

up-to-rhsTyEq2 : {δ δ' : DMor n (suc n)} → {{ctx (lhs δ) ≡ getCtx (ctx (rhs δ))}} →  {{ctx (lhs δ') ≡ getCtx (ctx (rhs δ'))}} → {{getMor δ ≡ getMor δ'}} → {{ctx (lhs δ) ≡ ctx (lhs δ')}} → {{ getMor δ ≡ idMor _}} → Derivable (ctx (lhs δ) ⊢ getTy (rhs δ) == getTy (rhs δ')) → Derivable (ctx (lhs δ) ⊢ getTm δ == getTm δ' :> getTy (rhs δ)) → proj {R = MorEquiv} δ ≡ proj δ'
up-to-rhsTyEq2 {δ = dmor' (dctx' dΓ) (dctx' {ctx = Γ' , A} (dΓ' , dA)) {mor = (δ , u)} (dδ , du)} {δ' = dmor' (dctx' dΔ) (dctx' {ctx = Δ' , B} (dΔ' , dB)) {mor = (δ' , u')} (dδ' , du')} {{refl}} {{refl}} {{refl}} {{refl}} {{refl}} dA= du= = eq (box (CtxRefl dΓ) (CtxRefl dΓ ,, dA=) (MorRefl dδ , congTmEq refl refl (! ([idMor]Ty _)) du=)) 

up-to-rhsTyEq2' : {δ δ' : DMor n (suc n)} → {{ctx (lhs δ) ≡ getCtx (ctx (rhs δ))}} →  {{ctx (lhs δ') ≡ getCtx (ctx (rhs δ'))}} → {{getMor δ ≡ getMor δ'}} → ⊢ ctx (lhs δ) == ctx (lhs δ') → (ctx (lhs δ) ⊢ getMor δ == idMor n ∷> getCtx (ctx (rhs δ))) → Derivable (ctx (lhs δ) ⊢ getTy (rhs δ) == getTy (rhs δ')) → Derivable (ctx (lhs δ) ⊢ getTm δ == getTm δ' :> getTy (rhs δ))  → proj {R = MorEquiv} δ ≡ proj δ'
up-to-rhsTyEq2' {δ = dmor' (dctx' dΓ) (dctx' {ctx = Γ' , A} (dΓ' , dA)) {mor = (δ , u)} (dδ , du)} {δ' = dmor' (dctx' dΔ) (dctx' {ctx = Δ' , B} (dΔ' , dB)) {mor = (δ' , u')} (dδ' , du')} {{refl}} {{refl}} {{refl}} dΓ= δ= dA= du= = eq (box dΓ= (dΓ= ,, dA=) (MorRefl dδ , ConvEq (TyEqTy1 dΓ dA=) du= (congTyEq ([idMor]Ty _) refl (SubstTyMorEq (TyEqTy1 dΓ dA=) (idMorDerivable dΓ) (MorSymm dΓ dΓ δ=))))) 
...
//-elimP-Ctx : ∀ {l} {C : (Γ : ObS n) → Prop l}
            → (proj* : (Γ : DCtx n) → C (proj (dctx (der Γ))))
            → (Γ : ObS n) → C Γ
//-elimP-Ctx proj* = //-elimP (λ {(dctx' dΓ) → proj* (dctx dΓ)})
...
dmorTmₛ {δ = δ@(dmor' (dctx' _) (dctx' {ctx = (_ , _)} (_ , _)) {mor = (_ , u)} _)} {{p}} {{q}} = S.is-section→ (DMor= refl (! p) (ap (λ z → weakenMor (idMor _)  [ z ]Mor) (Mor+= q refl) ∙ weakenMorInsert _ _ u ∙ idMor[]Mor _))

weakenObjS= : {Γ : DCtx n} {A : DCtx (suc n)} (A= : ftS (proj A) ≡ proj Γ) → S.weakenObj {A = proj A} A= ≡ proj (dctx ((der Γ , dTy A A=) , WeakTy (dTy A A=)))
